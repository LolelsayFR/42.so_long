...
void minimap_print_one(t_mlx_data *data, int x, int y, char c)
{
    ...
    cx = ((x - 4 * data->map->player_pos[0]) * 6 + (MINMAP_MX - MINMAP_X) / 2) + MINMAP_X + 9;
    cy = ((y - 4 * data->map->player_pos[1]) * 6 + (MINMAP_MY - MINMAP_Y) / 2) + MINMAP_Y + 9;
    if (!ft_strchr(MAP_SET, c) || !(cx > MINMAP_X && cx < MINMAP_MX) || !(cy > MINMAP_Y && cy < MINMAP_MY))
        return;
