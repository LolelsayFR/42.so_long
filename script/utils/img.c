...
/*   Updated: 2025/01/17 21:21:49 by emaillet         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	mlx_put_background(t_mlx_data *data)
{
	...
	if (!data->img->bg)
		sl_logs(data->logs->other, RED"[ERROR] Can't access background"RES);
...
void	mlx_put_tiles(t_mlx_data *data, t_img *tile, int x, int y)
{
	...
	if (!((x > 0 && x <= 11) && (y > 0 && y <= 11)))
		return ;
	if (!tile)
		sl_logs(data->logs->other, RED"[ERROR] Can't access tile data"RES);
...
void	mlx_put_hud_bg(t_mlx_data *data)
{
	...
	if (!data->img->hud_bg)
		sl_logs(data->logs->other, RED"[ERROR] Can't acces player data"RES);
...
void	mlx_put_player(t_mlx_data *data, int x, int y)
{
	...
	if (data->img->shield && data->player->hitted && (i % 10 < 2))
		draw_player(data->img->shield, data, x, y);
	if (data->player->taken || data->state == 1)
		draw_player(data->img->player_anim->take, data, x, y);
	else if (data->state == 2)
		draw_player(data->img->player_anim->dead, data, x, y);
	else if (data->img->player)
		draw_player(data->img->player, data, x, y);
	if (data->player->taken == 1)
		draw_player(data->img->potion, data, x + 14, y - 5);
	else if (data->player->taken == 2)
		draw_player(data->img->relic[data->frames % 8], data, x + 17, y - 5);
	else if (data->state == 1)
		draw_player(data->img->trophy, data, x + 16, y - 25);
	i++;
	if (i == FPS * 4)
		i = 0;
}
